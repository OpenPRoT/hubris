...
use drv_i2c_api::{Controller, I2cDevice, PortIndex, ResponseCode};
use ringbuf::*;
use userlib::*;
...
fn run_master_mode_tests(device: &I2cDevice) {
    ...
    uart_send(if read_result.is_ok() {
        b"PASS\r\n"
    } else {
        b"FAIL\r\n"
    });
    ...
    uart_send(if read_into_result.is_ok() {
        b"PASS\r\n"
    } else {
        b"FAIL\r\n"
    });
    ...
    uart_send(if block_result.is_ok() {
        b"PASS\r\n"
    } else {
        b"FAIL\r\n"
    });
    ...
    uart_send(if write_result.is_ok() {
        b"PASS\r\n"
    } else {
        b"FAIL\r\n"
    });
    ...
    let write_read_result: Result<u16, ResponseCode> =
        device.write_read_reg(0x40u8, &write_data_for_read);
    uart_send(if write_read_result.is_ok() {
        b"PASS\r\n"
    } else {
        b"FAIL\r\n"
    });
    ...
    let wr_block_result = device.write_read_block(
        0x50u8,
        &write_data_for_read,
        &mut wr_block_buffer,
    );
    uart_send(if wr_block_result.is_ok() {
        b"PASS\r\n"
    } else {
        b"FAIL\r\n"
    });
    ...
    uart_send(if write_write_result.is_ok() {
        b"PASS\r\n"
    } else {
        b"FAIL\r\n"
    });
    ...
    let wwr_result: Result<u32, ResponseCode> =
        device.write_write_read_reg(0x60u8, &write1_data, &write2_data);
    uart_send(if wwr_result.is_ok() {
        b"PASS\r\n"
    } else {
        b"FAIL\r\n"
    });
...
fn run_slave_mode_tests(device: &I2cDevice) {
    ...
    uart_send(if config_result.is_ok() {
        b"PASS\r\n"
    } else {
        b"FAIL\r\n"
    });
    ...
    uart_send(if enable_result.is_ok() {
        b"PASS\r\n"
    } else {
        b"FAIL\r\n"
    });
    ...
    uart_send(if check_result.is_ok() {
        b"PASS\r\n"
    } else {
        b"FAIL\r\n"
    });
    ...
    uart_send(if disable_result.is_ok() {
        b"PASS\r\n"
    } else {
        b"FAIL\r\n"
    });
...
fn uart_send(text: &[u8]) {
    ...
    let (code, _) =
        sys_send(uart_task, OP_WRITE, &[], &mut [], &[Lease::from(text)]);
    assert_eq!(0, code);
}
