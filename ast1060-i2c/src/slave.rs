...
impl<'a> Ast1060I2c<'a> {
    ...
    pub fn configure_slave(&mut self, config: &SlaveConfig) -> Result<(), I2cError> {
        ...
        self.regs().i2cc00().modify(|_, w| {
            w.enbl_master_fn().clear_bit()
        });
        
        // Set slave address
        self.regs().i2cs40().write(|w| unsafe {
            w.bits(u32::from(config.address & 0x7F))
        });
    ...
    pub fn slave_read(&mut self, buffer: &mut [u8]) -> Result<usize, I2cError> {
        ...
        if (status & AST_I2CS_PKT_ERROR) != 0 {
            self.clear_slave_interrupts();
            return Err(I2cError::SlaveError);
        }
        
        if (status & AST_I2CS_RX_DONE) == 0 {
            return Ok(0);
        }
        ...
        // Read from buffer
        copy_from_buffer(self.buff_regs(), &mut buffer[..to_read])?;
        
        // Clear RX_DONE interrupt
        unsafe {
            self.regs().i2cs24().write(|w| w.bits(AST_I2CS_RX_DONE));
        }
    ...
    pub fn slave_write(&mut self, data: &[u8]) -> Result<usize, I2cError> {
        ...
        // Copy data to buffer
        copy_to_buffer(self.buff_regs(), &data[..to_write])?;
    ...
    pub fn handle_slave_interrupt(&mut self) -> Option<SlaveEvent> {
        ...
        if (status & AST_I2CS_SLAVE_MATCH) != 0 {
            unsafe {
                self.regs().i2cs24().write(|w| w.bits(AST_I2CS_SLAVE_MATCH));
            }
            return Some(SlaveEvent::AddressMatch);
        }
        
        // Check for read request (master wants to read from us)
        if (status & AST_I2CS_WAIT_TX_DMA) != 0 {
        ...
        if (status & AST_I2CS_TX_ACK) != 0 {
            let len = self.regs().i2cs2c().read().bits() as usize;
            unsafe {
                self.regs().i2cs24().write(|w| w.bits(AST_I2CS_TX_ACK));
            }
            return Some(SlaveEvent::DataSent { len });
        }
        
        // Stop condition
        if (status & AST_I2CS_STOP) != 0 {
            unsafe {
                self.regs().i2cs24().write(|w| w.bits(AST_I2CS_STOP));
            }
            return Some(SlaveEvent::Stop);
        }
        
        // Packet done
        if (status & AST_I2CS_PKT_DONE) != 0 {
            unsafe {
                self.regs().i2cs24().write(|w| w.bits(AST_I2CS_PKT_DONE));
            }
        }
