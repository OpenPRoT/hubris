...
impl<'a> Ast1060I2c<'a> {
    ...
    /// Get access to buffer registers
    #[inline]
    fn buff_regs(&self) -> &BuffRegisterBlock {
    ...
    pub fn is_bus_busy(&self) -> bool {
        self.regs().i2cm14().read().master_bus_sts().bit_is_set()
    }
    
    /// Wait for completion with timeout
    fn wait_completion(&mut self, timeout_us: u32) -> Result<(), I2cError> {
...
fn init_i2c_global() -> Result<(), I2cError> {
    ...
    if I2CGLOBAL_INIT.compare_exchange(false, true, Ordering::SeqCst, Ordering::SeqCst).is_ok() {
        unsafe {
            ...
            // Reset I2C/SMBus controller
            scu.scu050().write(|w| w.rst_i2csmbus_ctrl().set_bit());
            
            // Small delay (need to add proper delay mechanism)
            for _ in 0..1000 {
                core::hint::spin_loop();
            }
