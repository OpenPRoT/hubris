...
impl<'a> Ast1060I2c<'a> {
    ...
    fn write_byte_mode(&mut self, addr: u8, bytes: &[u8]) -> Result<(), I2cError> {
        for (i, &byte) in bytes.iter().enumerate() {
            ...
            // Write data byte
            unsafe {
                self.regs().i2cm08().write(|w| w.bits(byte as u32));
            }
            ...
            // Wait for completion
            self.wait_completion(DEFAULT_TIMEOUT_US)?;
            
            // Check for errors
            let status = self.regs().i2cm14().read().bits();
            if status & AST_I2CM_TX_NAK != 0 {
            ...
            if is_last {
                unsafe {
                    self.regs().i2cm14().write(|w| w.bits(AST_I2CM_STOP_CMD));
                }
            }
        }
    ...
    fn read_byte_mode(&mut self, addr: u8, buffer: &mut [u8]) -> Result<(), I2cError> {
        for (i, byte) in buffer.iter_mut().enumerate() {
            ...
            // Read data
            let data = self.regs().i2cm08().read().bits();
    ...
    fn write_buffer_mode(&mut self, addr: u8, bytes: &[u8]) -> Result<(), I2cError> {
        ...
        while offset < bytes.len() {
            let chunk_len = core::cmp::min(BUFFER_MODE_SIZE, bytes.len() - offset);
            ...
            // Check for errors
            let status = self.regs().i2cm14().read().bits();
            if status & AST_I2CM_PKT_ERROR != 0 {
                if status & AST_I2CM_TX_NAK != 0 {
                    return Err(I2cError::NoAcknowledge);
                }
    ...
    fn read_buffer_mode(&mut self, addr: u8, buffer: &mut [u8]) -> Result<(), I2cError> {
        ...
        while offset < buffer.len() {
            let chunk_len = core::cmp::min(BUFFER_MODE_SIZE, buffer.len() - offset);
    ...
    pub fn handle_interrupt(&mut self) -> Result<(), I2cError> {
        ...
        if status & AST_I2CM_PKT_DONE != 0 {
            self.completion = true;
            self.clear_interrupts(AST_I2CM_PKT_DONE);
        ...
        if status & (AST_I2CM_TX_ACK | AST_I2CM_RX_DONE) != 0 {
            self.completion = true;
        ...
        if status & AST_I2CM_SCL_LOW_TO != 0 {
            self.clear_interrupts(status);
